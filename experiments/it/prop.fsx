...
Riferimenti
------------
In questa sezione esploriamo la logica proposizionale sfruttando il framework HOL di nholz. [Nholz](https://github.com/domasin/nholz) 
&egrave; semplicemente un portging in F# di [HOL Zero](http://www.proof-technologies.com/holzero/) che a sua volta &egrave; un dimostratore interattivo di teoremi sviluppato da Mark Adams 
in OCaml nello stile LCF della famiglia HOL.

Sfruttiamo il linguaggio definito da HolZero per esplorare la logica proposizionale con la guida 
dell'[Handbook of Practical Logic and Automated Reasoning](https://www.cl.cam.ac.uk/~jrh13/atp/) di John Harrison, 
seguendo passo passo il capitolo sulla logica proposizionale e riaddattando le funzioni l&igrave; definite al linguaggio HOL 
del nostro framework. 

L'Handbook di John Harrison &egrave; accompagnato 
da codice sorgente in OCaml che &egrave; stato portato in F# da Eric Taucher, Jack Pappas, Anh-Dung Phan ed &egrave; disponibile 
su Github: [fsharp-logic-examples](https://github.com/jack-pappas/fsharp-logic-examples/). Nel ridefinire le funzioni dell'handbook 
riaddattandole alla logica HOL si &egrave; utilizzata come riferimento proprio l'implementazione in F# del codice descritto nell'Handbook.
...
La logica proposizionale studia espressioni che intendono rappresentare proposizioni, cio&egrave; affermazioni che possono essere
considerate vere o false e che chiameremo nel seguito semplicemente "formule". All'interno del framework HOL che utiliziamo, queste 
sono semplicemente termini di tipo `bool` che possono essere costruite da atomi booleani, costituiti dalle costanti `true` e `false` 
e da variabili di tipo `bool`, a cui sono applicati i connettivi logici proposizionali `~`, `/\`, `\/`, `<=>` e `<=>`. Le proposizioni 
atomiche sono come le variabili nell'algebra ordinaria, e a volte ci riferiamo ad esse come variabili proposizionali o variabili booleane. 
Come suggerisce la parola "atomiche", non ne viene analizzata la loro struttura interna; questo porterebbe a conseiderare una logica 
predicativa che al momento non viene trattata. I connettivi proposizionali all'interno della logica HOL sono semplicemente funzioni da 
valori di verit&agrave; a valori di verit&agrave;.
...
e istruiamo l'interprete F# a restituire una raprresentazione concreta della sintassi dei tipi e dei termini piuttosto che la loro sintassi 
astratta interna al sistema:
...
Una prima cosa importante &egrave; poter distinguere tra espressioni atomiche ed esressioni composte. A questo scopo definiamo 
`is_bool_atom` come una funzione che restituisce vero per termini booleani costanti o variabili.
...
(**
Sulle formule composte vogliamo poter applicare delle funzioni sui loro atomi. 
A questo scopo definiamo `overatoms` per ricorsione su termini di questo genere come un analogo dell'iteratore di liste che 
itera una funzione binaria su tutti gli atomi di una formula.
*)
...
(**
Un'applicazione particolarmente comune &egrave; quella di raccogliere qualche insieme di attributi associati agli atomi; ritornando solamente, 
nel caso pi&ugrave; semplice, l'insieme di tutti gli atomi. Possiamo far questo iterando una funzione f insieme con un "append" su tutti gli 
atomi, e convertendo infine il risultato in un insieme per rimuovere i duplicati. 
*)
