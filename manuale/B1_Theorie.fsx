...
true_def                                                  // istanza della proprieta' riflessiva di uguaglianza per la funzione
// |- true <=> (\(p:bool). p) = (\p. p)                   // d'identita' booleana

conj_def                                                  // funzione binaria che restituisce se il fatto che i due argomenti
// |- $/\ = (\p1 p2. !p. (p1 ==> (p2 ==> p)) ==> p)       // inisieme implichino il valore, implica il valore

forall_def                                                // funzione che restituisce se il predicato restituisce true per ogni
// |- $! = (\(P:'a->bool). P = (\x. true))                // input

exists_def                                                // funzione che restituisce per un P se un qualsiasi elemento selezionato
// |- $? = (\(P:'a->bool). P ($@ P))                      // come soddisfacente il predicato necessariamente soddisfa il predicato

one_one_def                                               // funzione che restituisce se la funzione argomento e' iniettiva, cioe'
// |- ONE_ONE =                                           // se l'uguaglianza dei valori per due argomenti implica ncessariamente 
//     (\(f:'a->'b). !x1 x2. f x1 = f x2 ==> x1 = x2)     // l'uguaglianza dei due argomenti

type_definition_def                                       // funzione che prende un predicato per elementi del tipo di rappresenta-
// |- TYPE_DEFINITION =                                   // zione e un mapping da elementi del nuovo tipo al tipo di rappresenta-
//     (\P (rep:'b->'a). ONE_ONE rep                      // zione e restituisce se il mapping e' iniettivo e mappa su elementi che 
//                     /\ (!x. P x <=> (?y. x = rep y)))  // soddisfano il predicato. E' usata per definire nuove costanti di tipo

(**
**Assiomi**
*)

eta_ax                                                     // per ogni funzione f la lambda astrazione dell'applicazione di f a 
// |- !(f:'a->'b). (\x. f x) = f                           // alla variabile lambda e' uguale alla funzione stessa
   
imp_antisym_ax                                             // proprieta' antisimmetrica dell'implicazione
// |- !p1 p2. (p1 ==> p2) ==>                              // 
//              ((p2 ==> p1) ==> (p1 <=> p2))              // 
   
select_ax                                                  // per ogni P e x, se x soddisfa P, allora P e' soddisfatto anche 
// |- !(P:'a->bool) x. P x ==> P ($@ P)                    // dall'elemento selezionato per P
