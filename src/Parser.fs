                    ...
                    (!|||)
                        (
                            ...
                            (fun x -> let x0 = (char_implode <* filter is_digit <* char_explode) x in
                                      mk_bigint_nat_preterm (BigInteger.Parse(x0))) @|
                               parse_name_with is_numeral_token
                        )
                        (
                            (!|||)
...
(* be opened or included in order for parsing to work).  This is because the  *)
(* language would otherwise module-prefix the ML type label of any type and   *)
(* term quotations entered as top-level ML expressions.                       *)

let string_tail (x:string) i = x.Substring(i, x.Length - i)

let rec skip_space (x:string) i =
    if (i < x.Length) && (is_whitespace_char (x.[i]))
    then skip_space x (i+1)
    else i

let expand_hol_quotation (x:string) =
